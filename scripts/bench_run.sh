...
build_mysql()
{
  ...
  if test "x$ERROR_INSERT_FLAG" = "xyes" ; then
    ERROR_INSERT_FLAG="1"
  else
    ERROR_INSERT_FLAG="0"
  fi
  BUILD_COMMAND="cmake"
  BUILD_COMMAND="$BUILD_COMMAND -DWITH_DEBUG=$DEBUG_FLAG"
  BUILD_COMMAND="$BUILD_COMMAND -DWITH_ERROR_INSERT=$ERROR_INSERT_FLAG"
