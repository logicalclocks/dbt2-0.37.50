...
#
# 8 jan 2015
# Adapted calculations of sleep time to be correct, slept for too long
# before.
# 
# 27 mar 2015
# Added handling of DYLD_LIBRARY_PATH for Mac OS X
#
...
# Determine run number for selecting an output directory
  if [ "x$RUN_NUMBER" = "x" ] ; then
    RUN_NUMBER=-1
    if [ -f "/$DEFAULT_DIR/run_number" ]; then 
      read RUN_NUMBER < /$DEFAULT_DIR/run_number
    fi
...
# Update the run number for the next test.
    ...
    echo $NEW_RUN_NUMBER > /$DEFAULT_DIR/run_number
  fi
}
...
do
  case $1 in
    ...
    --tot_warehouses )
      opt=$1
      shift
      TOT_WAREHOUSES=`echo $1 | egrep "^[0-9]+$"`
      validate_parameter $opt $1 $TOT_WAREHOUSES
      ;;
...
if test "x$DYLD_LIBRARY_PATH" = "x" ; then
  DYLD_LIBRARY_PATH="$LIB_CLIENT_PATH"
else
  DYLD_LIBRARY_PATH="$DYLD_LIBRARY_PATH:$LIB_CLIENT_PATH"
fi
...
NUM_WARES=$(($LAST_WAREHOUSE - FIRST_WAREHOUSE + 1))
((NUM_WARES /= SPREAD ))
THREADS=$(( $NUM_WARES * $TPW ))
WARMUPTIME=$(( $THREADS * $SLEEPY ))
((WARMUPTIME /= 1000))
((WARMUPTIME += 4))
...
if [ "x$REAL_RUN" = "xyes" ] ; then
  ...
  DRIVER_COMMAND_ARGS="$DRIVER_COMMAND_ARGS $DURATION"
  DRIVER_COMMAND_ARGS="$DRIVER_COMMAND_ARGS -tpw $TPW"
  DRIVER_COMMAND_ARGS="$DRIVER_COMMAND_ARGS -wmin $FIRST_WAREHOUSE"
  DRIVER_COMMAND_ARGS="$DRIVER_COMMAND_ARGS -wmax $LAST_WAREHOUSE"
  DRIVER_COMMAND_ARGS="$DRIVER_COMMAND_ARGS -spread $SPREAD"
  ...
  if [ "x$USE_ALTERED_MODE" = "x1" ]; then
    DRIVER_COMMAND_ARGS="$DRIVER_COMMAND_ARGS -altered 1"
    DRIVER_COMMAND_ARGS="$DRIVER_COMMAND_ARGS -w $TOT_WAREHOUSES"
  else
...
# Run some post processing analysese.
  ...
  perl $DBT2_PATH/scripts/mix_analyzer.pl --infile $DRIVER_OUTPUT_DIR/mix.log --outfile $DRIVER_OUTPUT_DIR/statistics.log
